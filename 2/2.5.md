...
struct either_int_string either_from_string(char* s) {
    struct either_int_string result = {.type = ET_STRING, .as_string = s};
    return result;
}
struct either_int_string either_from_int(int64_t i) {
    struct either_int_string result = {.type = ET_INT, .as_int = i};
    return result;
}
...
### > Реализуйте упакованный указатель на строку...
```c
struct heap_string {
  char* addr;
};
...
struct heap_string halloc( const char* s ) {
    ...
    struct heap_string result = {.addr = new_s};
    return result;
}
