...
struct maybe_int64 array_int_min( struct array_int array ) {
    if (array.size > 0) {
        int64_t min_ptr = array.data[0];
        for (size_t i = 0; i < array.size; i++) {
            if (min_ptr > array.data[i]) {min_ptr = array.data[i];}
        }
        return some_int64(min_ptr);
    }
    return none_int64;
}
...
struct array_array_int array_array_int_read() {
    ...
    for (size_t i = 0; i < rows; i++) {
        array_array_int_set_row(aarray, i, array_int_read());
    }
    return aarray;
}
...
void array_array_int_print( struct array_array_int array) {
    for (size_t i = 0; i < array.size; i++) {
        array_int_print(array.data[i]); // todo: empty ???
        printf("\n");
    }
}
...
struct maybe_int64 array_array_int_min( struct array_array_int array ) {
    ...
    for (size_t i = 0; i < array.size; i++) {
        ...
        if (row.valid) {
            min = maybe_int64_min(array_int_min(row.value), min);
        }
    }
    return min;
}
...
void array_array_int_normalize( struct array_array_int array, int64_t m) {
    struct maybe_array_int row;
    for (size_t i = 0; i < array.size; i++) {
        row = array_array_int_get_row(array, i );
        if (row.valid) {
            array_int_normalize(row.value, m );
        }
    }
}
...
### > Мы уже упоминали аппаратный стек, хранящий...
```c
...
struct stack stack_create( size_t size ) {
    struct stack result = {.count = 0, .data = array_int_create(size)};
    return result;
}
